...
export default function EditPostPage({ params }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const updatedPost = {
      ...
      content,
      quest:
        questQuestion && questOptions.every(opt => opt.length > 0) && questAnswerIndex !== "" && questXp !== ""
          ? {
    ...
    <div className="max-w-4xl mx-auto bg-[#2c313a] p-8 sm:p-12 rounded-2xl shadow-lg border border-[#3a404d] text-white animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {content.map((block, index) => (
          <div key={index} className="bg-[#3a404d] p-4 rounded-lg border border-[#4a505c] relative">
            ...
            {block.type === "paragraph" && (
              <div>
                <label htmlFor={`content-paragraph-${index}`} className="block text-sm font-medium mb-2">
                  내용
                </label>
                <textarea
            ...
            {block.type === "dialogue" && (
              <div className="space-y-4">
                <div>
                  <label htmlFor={`content-character-${index}`} className="block text-sm font-medium mb-2">
                    캐릭터
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label htmlFor={`content-avatar-${index}`} className="block text-sm font-medium mb-2">
                    아바타 URL
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label htmlFor={`content-dialogue-text-${index}`} className="block text-sm font-medium mb-2">
                    대화 내용
                  </label>
                  <textarea
        ...
        <div className="flex space-x-4 mt-4">
          ...
          <button
            type="button"
            onClick={() => addContentBlock("dialogue")}
        ...
        <div className="bg-[#3a404d] p-6 rounded-lg border border-[#4a505c] space-y-4">
          <div>
            <label htmlFor="questQuestion" className="block text-lg font-medium mb-2">
              질문
            </label>
            <input
              type="text"
          ...
          <div>
            <label htmlFor="questAnswerIndex" className="block text-lg font-medium mb-2">
              정답 인덱스 (0-3)
            </label>
            <input
              type="number"
          ...
          <div>
            <label htmlFor="questXp" className="block text-lg font-medium mb-2">
              XP
            </label>
            <input
              type="number"
